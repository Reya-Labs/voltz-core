...
import { Deployment, validAddress, deployWithName, getContractFromDeployment, getCreate2Address } from '../helpers/deployHelpers';
...
export async function step1(deployer: any, hre: any, deployment: Deployment, consts: any) {
    ...
    const factory: Factory = await getContractFromDeployment(hre, deployment, "Factory");
    await factory.createAMM(consts.tokens.USDC.address, utils.formatBytes32String("AaveV2"), toBn(termEndTimestamp.toString()));
    ...
    const ammAddress = getCreate2Address(factory.address, utils.formatBytes32String("AaveV2"), consts.tokens.USDC.address, termStartTimestampBN, termEndTimestampBN, ammBytecode);
