  ...
  for (const ammCase of TEST_AMMs) {
    describe(ammCase.description, () => {
      ...
      const ammCaseFixture = async () => {
        ...
        const {factory, swapTargetCallee: swapTarget, createAMM } = await AMMFixture(
          [wallet],
          waffle.provider
        )
        ...
        for (const position of ammCase.positions) {
          await ammFunctions.mint(wallet.address, position.tickLower, position.tickUpper, position.liquidity)
        }
    ...
    beforeEach('load fixture', async () => {
      ;({ amm, ammFunctions, ammBalance0, ammBalance1, swapTarget } = await loadFixture(
        ammCaseFixture
      ))
    })
    ...
    for (const testCase of ammCase.swapTests ?? DEFAULT_POOL_SWAP_TESTS) {
      it(swapCaseToDescription(testCase), async () => {
        ...
        const executionPrice = new Decimal(ammBalance1Delta.toString()).div(ammBalance0Delta.toString()).mul(-1) // todo: why mul(-1) ?
