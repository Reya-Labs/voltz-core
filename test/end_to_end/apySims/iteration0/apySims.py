...
with open('test/end_to_end/apySims/iteration0/console.txt') as f:
    lines = f.readlines()
    phases = []
    for l in lines:
        if l.startswith("---"):
...
file = open("test/end_to_end/apySims/iteration0/sim_info.json", "w")
file = json.dump(phase_information, file)
# file = open("test/end_to_end/apySims/iteration0/sim_info.json", "r")
...
apyDataFrame.to_csv('test/end_to_end/apySims/iteration0/apys.csv')
...
for name in phase_information.keys():
    if phase_information[name]["current timestamp"] < phase_information[name]["end timestamp"]:
        ...
        position_margin.append(
            phase_information[name]["position info"][0]["margin"])
        margin_requirement.append(
            phase_information[name]["position info"][0]["margin requirement"])
        liquidation_threshold.append(
            phase_information[name]["position info"][0]["liquidation threshold"])
...
marginDataFrame.to_csv('test/end_to_end/apySims/iteration0/margins.csv')
