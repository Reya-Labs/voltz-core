...
describe("Periphery", async () => {
  ...
  beforeEach("deploy fixture", async () => {
    ({ token, vammTest, marginEngineTest, factory, testFixedAndVariableMath } =
  ...
  it("FC VT Swap: No slippage, over collateralised", async () => {
    ...
    const rateOracle = (await ethers.getContractAt(
      "TestRateOracle",
      await marginEngineTest.rateOracle()
    )) as TestRateOracle;

    const variableFactorWad = await rateOracle.callStatic.variableFactor(
      await marginEngineTest.termStartTimestampWad(),
      await marginEngineTest.termEndTimestampWad()
    );

    // OVER-COLLATERALISED FC VT SWAP
    await periphery.connect(other).callStatic.fullyCollateralisedVTSwap(
      {
        marginEngine: marginEngineTest.address,
        isFT: false,
        notional: toBn("10"),
    ...
    const fcVTMarginReq =
      ...
      Number(
        ethers.utils.formatEther(
          (
            await testFixedAndVariableMath.fixedFactorTest(
              true,
              await marginEngineTest.termStartTimestampWad(),
              await marginEngineTest.termEndTimestampWad()
            )
  ...
  it("FC VT Swap: No slippage, just under fully collateralised", async () => {
    ...
    await expect(
      periphery.connect(other).callStatic.fullyCollateralisedVTSwap(
        {
          marginEngine: marginEngineTest.address,
          isFT: false,
          notional: toBn("10"),
          ...
          tickLower: -TICK_SPACING,
          tickUpper: TICK_SPACING,
          marginDelta: toBn((fcVTMarginReq - 0.00001).toString()),
        },
        variableFactorWad
      )
        ...
        ethers.utils.formatEther(
          (
            await testFixedAndVariableMath.fixedFactorTest(
              ...
              (
                await marginEngineTest.termStartTimestampWad()
              ).add(utils.parseEther(consts.ONE_DAY.mul(3).toString())),
              await marginEngineTest.termEndTimestampWad()
            )
          ).toString()
        )
