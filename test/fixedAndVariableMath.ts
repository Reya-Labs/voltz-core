...
function getFixedTokenBalance(amount0: BigNumber, amount1: BigNumber, variableFactorAccrued: BigNumber, termStartTimestamp: BigNumber, termEndTimestamp: BigNumber) : BigNumber {
  ...
  const fixedFactorAccrued: BigNumber = fixedFactor(false, termStartTimestamp, termEndTimestamp)
  
  const excessFixedAccruedBalance: BigNumber = mul(amount0, fixedFactorAccrued)

  const excessVariableAccruedBalance: BigNumber = mul(amount1, variableFactorAccrued)

  const excessBalance: BigNumber = add(excessFixedAccruedBalance, excessVariableAccruedBalance)

  const fixedTokenBalance = calculateFixedTokenBalance(amount0, excessBalance, termStartTimestamp, termEndTimestamp)
...
function calculateFixedTokenBalance(amount0: BigNumber, exceessBalance: BigNumber, termStartTimestamp: BigNumber, termEndTimestamp: BigNumber) : BigNumber {

  const fixedFactorAtMaturity: BigNumber = fixedFactor(true, termStartTimestamp, termEndTimestamp)
...
describe("FixedAndVariableMath", () => {
  ...
  describe("#calculateFixedTokenBalance", () => {
    ...
    it("correctly calculates the fixed token balance", async () => {
        ...
        const expected: BigNumber = await calculateFixedTokenBalance(amount0, excessBalance, termStartTimestamp, termEndTimestamp)

        expect(await fixedAndVariableMath.calculateFixedTokenBalance(amount0, excessBalance, termStartTimestamp, termEndTimestamp)).to.eq(expected)
  ...
  describe("#getFixedTokenBalance", () => {
    ...
    it("correctly gets the fixed token balance", async () => {
        ...
        const termEndTimestamp: BigNumber = add(toBn(BLOCK_TIMESTAMP.toString()), toBn("7776000"))
        const termStartTimestamp: BigNumber = sub(toBn(BLOCK_TIMESTAMP.toString()), toBn("7776000"))

        const expected: BigNumber = getFixedTokenBalance(amount0, amount1, variableFactorAccrued, termStartTimestamp, termEndTimestamp)

        expect(await fixedAndVariableMath.getFixedTokenBalance(amount0, amount1, variableFactorAccrued, termStartTimestamp, termEndTimestamp)).to.eq(expected)
