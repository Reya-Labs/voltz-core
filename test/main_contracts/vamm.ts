...
describe("VAMM", () => {
  ...
  describe("#initialize", () => async () => {
    it('fails if already initialized', async () => {
      ...
      await expect(vammTest.initialize(encodeSqrtRatioX96(1, 1).toString())).to.be.reverted;
    })
    ...
    it("fails if starting price is too high", async () => {
      ...
      await expect(vammTest.initialize(BigNumber.from(2).pow(160).sub(1))).to.be.reverted;
    })
  ...
  describe("#mint", () => {
    it('fails if not initialized', async () => {
      await expect(vammTest.mintTest(wallet.address, -tickSpacing, tickSpacing, 1)).to.be.reverted;
    })
    // more tests in here
