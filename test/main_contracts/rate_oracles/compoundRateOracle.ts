...
describe("Compound Rate Oracle", () => {
  ...
  const oracleFixture = async () => {
    ...
    //   BigNumber.from(10).pow(26).mul(2).toString()
    // );
    ...
    return {
      testRateOracle: compoundRateOracleTest,
      token: _token,
      cToken: _cToken,
      compoundRateDecimals: 28,
    };
  };
  ...
  describe("#initialize", () => {
    beforeEach("deploy and initialize test oracle", async () => {
      ...
      const cToken = await ethers.getContract("MockCToken");
  ...
  describe("#write", () => {
    beforeEach("deploy and initialize test oracle", async () => {
      ({ testRateOracle, cToken } = await loadFixture(oracleFixture));
    ...
    it("single element array gets overwritten", async () => {
      ...
      expect(rateValue).to.eq(toBn("0.02", consts.NORMALIZED_RATE_DECIMALS));
    ...
    it("grows cardinality if writing past", async () => {
      ...
      expect(rateValue).to.eq(
        toBn(
          consts.COMPOUND_DAY0_EXCHANGE_RATE_AS_DECIMAL,
  ...
  describe("#getRateFromTo", async () => {
    beforeEach("deploy and initialize test oracle", async () => {
      ({ testRateOracle, token, cToken } = await loadFixture(oracleFixture));
    });
    ...
    it("correctly calculates rate from one timestamp to the next", async () => {
      ...
      await cToken.setExchangeRate(
        toBn(
          consts.COMPOUND_DAY0_EXCHANGE_RATE_AS_DECIMAL * 1.1,
          COMPOUND_RATE_DECIMALS
        )
      );
      await testRateOracle.writeOracleEntry();
      ...
      expect(rateFromTo).to.be.closeTo(expectedRateFromTo, 10_000_000_000_000); // within 1e13 of 1e17 = within 0.01%
    });
    ...
    it("correctly calculates rate from one timestamp to the next when there was no recent write", async () => {
      ...
      await cToken.setExchangeRate(
        toBn(
          consts.COMPOUND_DAY0_EXCHANGE_RATE_AS_DECIMAL * 1.21,
          COMPOUND_RATE_DECIMALS
        ) // Factor of 1.1 every 500 days
      );
      const rateToTimestamp = await getCurrentTimestamp(provider);
  ...
  describe("#getApyFromTo", async () => {
    const oneInRay = toBn("1", consts.NORMALIZED_RATE_DECIMALS);
    const expectedApy = toBn(
      "1.3707526909509977",
      consts.NORMALIZED_RATE_DECIMALS
    ); // This is equivalent to compounding by 1.00000001 per second for 365 days = 31536000 seconds
    ...
    beforeEach("deploy and initialize test oracle", async () => {
      ...
      await cToken.setExchangeRate(oneInRay);
      await testRateOracle.writeOracleEntry();
      startTime = await getCurrentTimestamp();

      // One year passes
      await cToken.setExchangeRate(expectedApy);
      await setTimeNextBlock(startTime + 31536000); // One year after first reading
      await testRateOracle.writeOracleEntry();

      // Another year passes
      await cToken.setExchangeRate(twoYearMultiple);
  ...
  describe("#getSurroundingRates", async () => {
    ...
    beforeEach("deploy and initialize test oracle", async () => {
      ...
      await advanceTimeAndBlock(BigNumber.from(86400), 2); // advance by one day
      // set new liquidity index value
      await cToken.setExchangeRate(toBn("1.1", COMPOUND_RATE_DECIMALS));
  ...
  describe("#variableFactor", async () => {
    ...
    it("calculates variable factor before maturity", async () => {
      ...
      await cToken.setExchangeRate(
        toBn(
          consts.COMPOUND_DAY0_EXCHANGE_RATE_AS_DECIMAL * 1.2,
          COMPOUND_RATE_DECIMALS
        )
      );
