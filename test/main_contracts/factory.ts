...
import { timeFixture, metaFixture, fixedAndVariableMathFixture, marginCalculatorFixture, rateOracleFixture, mockAaveLendingPoolFixture } from "../shared/fixtures";
import { getCurrentTimestamp } from "../helpers/time";
import { toBn } from "evm-bn";
import { consts } from "../helpers/constants";
import { ERC20Mock, FixedAndVariableMath, MockAaveLendingPool, TestRateOracle, Time } from "../../typechain";
import { RATE_ORACLE_ID, INVALID_ORACLE_ID, ZERO_ADDRESS, ZERO_BYTES } from "../shared/utilities"
...
describe("Factory", () => {
    ...
    describe("#updateCalculator", () => {
        ...
        it("check calculator", async () => {
            ...
            expect(await factory.calculator()).to.not.equal(testMarginCalculator.address);
    ...
    describe("#createAMM", () => {
        ...
        it("checkOwnerPrivilege", async () => {
            await expect(factory.connect(other).createAMM(token.address, RATE_ORACLE_ID, termEndTimestampBN)).to.be.revertedWith("NOT_OWNER");
        });

        it("checkCreateAMM", async () => {
            // create AMM
            let tx = await factory.createAMM(token.address, RATE_ORACLE_ID, termEndTimestampBN);
    ...
    describe("#addRateOracle", () => {
        ...
        beforeEach("deploy fixture", async() => {
            ...
            ({ fixedAndVariableMath } = await fixedAndVariableMathFixture(time.address));
            ({ factory, token} = await loadFixture(metaFixture));
        ...
        it("checkOwnerPrivelege", async () => {
            await expect(factory.connect(other).addRateOracle(RATE_ORACLE_ID, testRateOracle.address)).to.be.revertedWith("NOT_OWNER");
        });

        it("checkZeroBytes", async () => {
            await expect(factory.addRateOracle(ZERO_BYTES, testRateOracle.address)).to.be.revertedWith("ZERO_BYTES");
        });

        it("checkInvalidOracleID", async () => {
            await expect(factory.addRateOracle(INVALID_ORACLE_ID, testRateOracle.address)).to.be.revertedWith("INVALID_ID");
        });

        it("checkExistedID", async () => {
            await expect(factory.addRateOracle(RATE_ORACLE_ID, testRateOracle.address)).to.be.revertedWith("EXISTED_ID");
        });
    });
});