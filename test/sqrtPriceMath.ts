...
describe('SqrtPriceMath', () => {
    ...
    describe('#getAmount0Delta', () => {
        ...
        it('returns 0 if liquidity is 0', async () => {
            const amount0 = await sqrtPriceMath.getAmount0Delta(encodeSqrtRatioX96(1, 1).toString(), encodeSqrtRatioX96(2, 1).toString(), 0, true)
    
            expect(amount0).to.eq(0)
        })

        it('returns 0 if prices are equal', async () => {
            const amount0 = await sqrtPriceMath.getAmount0Delta(encodeSqrtRatioX96(1, 1).toString(), encodeSqrtRatioX96(1, 1).toString(), 0, true)
    ...
    describe('#getAmount1Delta', () => {
        it('returns 0 if liquidity is 0', async () => {
          const amount1 = await sqrtPriceMath.getAmount1Delta(encodeSqrtRatioX96(1, 1).toString(), encodeSqrtRatioX96(2, 1).toString(), 0, true)
