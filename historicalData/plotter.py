...
def parseSettings():
    parser.add_argument("-asset", "--asset", type=str, help="Asset for plotter (e.g. --asset rETH)", required=True)
    ...
    LOOKBACK_WINDOWS = [1 * SECONDS_IN_HOUR, 6 * SECONDS_IN_HOUR, 24 * SECONDS_IN_HOUR, 7 * SECONDS_IN_DAY, 3 * 7 * SECONDS_IN_DAY]
    LOOKBACK_WINDOW_LABELS = ["1 hour", "6 hours", "1 day", "1 week", "3 weeks"]
...
def getDailyApy(df, lookback, frequency):
    ...
    print("Creating a dataset between {0} and {1}.".format(datetime.datetime.utcfromtimestamp(start_range), datetime.datetime.utcfromtimestamp(end_range)))
...
def getDatasets():
    ...
    apys = [getDailyApy(rnis, lookback=lookback, frequency = APY_FREQUENCY) for lookback in LOOKBACK_WINDOWS]
...
def plot(rnis, apys):
    ...
    fig.canvas.manager.set_window_title("Historical data for {0}".format(ASSET))

    axs[0, 0].set_title("Raw rates")
    axs[0, 0].set_xticks([rnis["timestamp"][0], rnis["timestamp"][len(rnis["timestamp"]) - 1]])
    axs[0, 0].set_xticklabels([datetime.date.fromtimestamp(rnis["timestamp"][0]), datetime.date.fromtimestamp(rnis["timestamp"][len(rnis["timestamp"]) - 1])])
    ...
    for [t, i, j] in indices:
        ...
        axs[i, j].set_xticklabels([datetime.date.fromtimestamp(start_timestamp), datetime.date.fromtimestamp(end_timestamp)])

        axs[i, j].set_ylim(APY_LIMITS)

        yticks = [i for i in np.linspace(APY_LIMITS[0], APY_LIMITS[1], 6)]
        axs[i, j].set_yticks(yticks)
        axs[i, j].set_yticklabels(['{0}%'.format(int(100 * i)) for i in yticks])
