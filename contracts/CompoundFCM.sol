...
contract CompoundFCM is CompoundFCMStorage, IFCM, ICompoundFCM, Initializable, OwnableUpgradeable, PausableUpgradeable, UUPSUpgradeable {
  ...
  function initialize(IVAMM __vamm, IMarginEngine __marginEngine) external override initializer {
    ...
    _underlyingYieldBearingToken = IERC20Minimal(underlyingToken); // ctoken or underlying?
  ...
  function initiateFullyCollateralisedFixedTakerSwap(uint256 notional, uint160 sqrtPriceLimitX96) external override {
    ...
    // deposit notional executed in terms of cTokens (e.g. cDAI) to fully collateralise your position
  ...
  function settleTrader() external override onlyAfterMaturity returns (int256 traderSettlementCashflow) {
    ...
    if (settlementCashflow < 0) {
      uint256 currentExchangeRate = _ctoken.exchangeRateStored();
      uint256 updatedTraderMarginInScaledYieldBearingTokens = trader.marginInScaledYieldBearingTokens - uint256(-settlementCashflow).rayDiv(currentExchangeRate);
