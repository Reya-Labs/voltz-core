...
contract MockAaveV3LendingPool is IAaveV3LendingPool {
    ...
    returns (uint256)
  {
     ...
     console.log("Last updated timestamp", _reserves[asset].lastUpdateTimestamp);
    ...
    if (timestamp == block.timestamp) {
      ...
      return
        calculateLinearInterest(_reserves[asset].currentLiquidityRate, timestamp).rayMul(
  ...
  function setReserveNormalizedIncome(
        ...
        _reserves[_underlyingAsset].currentLiquidityRate = reserveNormalizedIncome;
        require((block.timestamp == uint40(block.timestamp)), "TSOFLOW");
        _reserves[_underlyingAsset].lastUpdateTimestamp = uint40(block.timestamp);
