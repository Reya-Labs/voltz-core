...
contract Periphery {
  ...
  function getMarginEngine(address marginEngineAddress) internal pure returns (IMarginEngine) {
    IMarginEngine marginEngine = IMarginEngine(marginEngineAddress);
    return marginEngine;
  }

  function getVAMM(address marginEngineAddress) internal view returns (IVAMM) {
  ...
  function mintOrBurn(MintOrBurnParams memory params) external {
    ...
    uint128 liquidity = LiquidityAmounts.getLiquidityForAmount1(sqrtRatioAX96, sqrtRatioBX96, params.notional);
  ...
  function swap(SwapPeripheryParams memory params) external {
    ...
    IVAMM.SwapParams memory swapParams = IVAMM.SwapParams({
      ...
      sqrtPriceLimitX96:  params.sqrtPriceLimitX96 == 0
        ? (!params.isFT ? TickMath.MIN_SQRT_RATIO + 1 : TickMath.MAX_SQRT_RATIO - 1)
